defmodule Ex3 do
  ...
  [2, 4, 6, 8, 10]
  """
  def map([], _f), do: [] # clausula 1: lista vazia
  def map(_h, []), do: [] # clausula 2: função vazia
  def map([], []), do: [] # clausula 3: lista e função vazia
  # Pega o head da lista e aplica a função
  # Passa o resto da lista como parâmetro da função
  # passando por recursividade até a lista acabar
  def map([h | r], f), do: [ f.(h) | map(r, f)]
  ...
  def fold_right([h | r], ini, f), do: f.(h, fold_right(r, ini, f))
  ...
  "foobarbaz"
  """
  # def concatena_strings([], _n), do: "" # continua retornando []
  def concatena_strings(ls) do
    if ls == [] do
      " "
    else
      fold_right(ls, "", fn x, y -> x <> y end)
    end
  end
  ...
  def fold_left([h | r], ini, f), do: fold_left(r, f.(ini, h), f)
  ...
  def concat_strings_sep([h | r], sep), do: h <> fold_left(r, "", fn x, y -> x <> sep <> y end)
  ...
  [1, 3, 5]
  """
  def par_3n_mais_1(l), do: filter(l , fn x -> num_par(x*3 + 1) end)
  def num_par(n), do: rem(n, 2) == 0
end
